...
function DiaryScreen({ onNav = () => {}, onSample = () => {}, trackingDays = 14, scansSaved = 32, autoEntries = true, calendarActivity = 50 }) {
  ...
  return (
    <PhoneScreen bg={T.bg} scroll={false}>
      ...
      <div className="fs-scroll" style={{ flex: 1, padding: '0 18px 18px' }}>
        ...
        {/* sample scan — runs the real matcher on the demo product */}
        <button className="fs-press" onClick={onSample}
          style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 11, padding: '13px 15px', marginBottom: 16, borderRadius: 15,
            background: T.accent, color: '#FBF7EF', boxShadow: '0 2px 10px rgba(35,74,71,.22)', textAlign: 'left' }}>
          <span style={{ width: 34, height: 34, borderRadius: 10, flex: '0 0 34px', display: 'grid', placeItems: 'center', background: 'rgba(255,255,255,.18)' }}>
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="#FBF7EF" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="8" cy="8" r="5.5"/><path d="M12 12l3.5 3.5"/></svg>
          </span>
          <span style={{ flex: 1 }}>
            <span style={{ display: 'block', fontFamily: T.sans, fontWeight: 700, fontSize: 14.5 }}>Try a sample scan</span>
            <span style={{ display: 'block', fontFamily: T.sans, fontSize: 12.5, opacity: .85, marginTop: 1 }}>See a real result on Sunflower Seed Cookies</span>
          </span>
        </button>
        ...
        <div style={{ background: T.surface, borderRadius: 18, boxShadow: T.shadowSoft, padding: '16px 16px 18px', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['‹', '›'].map((a) => <button key={a} className="fs-press" onClick={() => window.openInfo({ title: 'Browse other months', sub: 'Step back and forward through your diary by month.', lines: 2 })} style={{ width: 32, height: 32, borderRadius: 9, background: T.surfaceWarm, color: T.ink2, fontSize: 16, boxShadow: `inset 0 0 0 1px ${T.line}` }}>{a}</button>)}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4 }}>
            ...
            {days.map((d) => {
              ...
              const opens = isToday || tag;
              return (
                <div key={d} onClick={opens ? () => window.openInfo({ title: `${d} June 2026`, sub: 'The scans you saved on this day.', lines: 3 }) : undefined}
                  style={{ aspectRatio: '1', borderRadius: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3,
          ...
          {[
            ...
            <div key={i} style={{ background: T.surface, borderRadius: 15, boxShadow: T.shadowSoft, padding: '13px 15px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button className="fs-press" onClick={() => window.openInfo({ title: 'Rename this entry', sub: 'Give this scan a name you’ll recognise later.', lines: 2 })} style={{ fontFamily: T.sans, fontSize: 12.5, fontWeight: 600, color: T.accent, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              ...
              <button className="fs-press" onClick={() => window.openInfo({ title: 'Add a note', sub: 'Jot down a thought about this product for future you.', lines: 2 })} style={{ marginTop: 9, height: 36, padding: '0 13px', borderRadius: 10, fontFamily: T.sans, fontSize: 13, fontWeight: 600, color: T.ink2, background: T.surfaceWarm, boxShadow: `inset 0 0 0 1px ${T.line}`, display: 'inline-flex', alignItems: 'center', gap: 7 }}>
    ...
    <PhoneScreen bg={T.bg} scroll={false}>
      ...
      <div className="fs-scroll" style={{ flex: 1, padding: '0 22px 18px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', marginTop: 12 }}>
          {rows.map((r, i) => (
            <div key={r.name} onClick={() => window.openInfo({ title: r.name, sub: `Appeared in ${r.n} of your scans in the last 30 days.`, lines: 3 })} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 13, padding: '13px 0', borderBottom: i < rows.length - 1 ? `1px solid ${T.lineSoft}` : 'none' }}>
...
function TokenCard({ remaining = 7, total = 20 }) {
  ...
  return (
    <div style={{ borderRadius: 16, background: zero ? T.surfaceWarm : T.surface, boxShadow: T.shadowSoft, padding: '16px 17px' }}>
      ...
      {zero && <div style={{ marginTop: 13 }}><Btn size="md" onClick={() => window.openInfo({ title: 'Add more scans', sub: 'Top up this month’s scans. Pricing is still being decided.', lines: 2 })}>Add more scans</Btn></div>}
    </div>
  );
}
...
function SRow({ icon, label, sub, value, danger, toggle, on, onToggle, last, onClick }) {
  const interactive = !toggle;
  const fg = danger ? T.intoler.strong : T.ink;
  return (
    <div className={interactive ? 'fs-tap' : ''} onClick={toggle ? () => onToggle(!on) : onClick}
    ...
    <PhoneScreen bg={T.bg} scroll={false}>
      ...
      <div className="fs-scroll" style={{ flex: 1, padding: '0 22px 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 22 }}>
          ...
          <button className="fs-press" onClick={() => window.openInfo({ title: 'Add a profile', sub: 'Create another family member to scan for — adult or child.', lines: 3 })} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
        ...
        {[
          ...
          <div key={g.t} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              ...
              <button className="fs-press" onClick={() => window.openInfo({ title: `${g.items.length ? 'Edit' : 'Add'} ${g.t.toLowerCase()}`, sub: `Choose what ${active} should have flagged under ${g.t.toLowerCase()}.`, lines: 3 })} style={{ fontFamily: T.sans, fontSize: 13, fontWeight: 600, color: T.accent }}>{g.items.length ? 'Edit' : 'Add'}</button>
            </div>
            {g.items.length ? (
        ...
        <SGroup title="Account">
          <SRow icon="mail" label="Email" sub="Syncs your profile & diary" value="maya@…"
            onClick={() => window.openInfo({ title: 'Email', sub: 'The address used to save your profile and sync your diary across devices. Change it or sign out from here.', lines: 3 })} />
          <SRow icon="people" label="Family allergen session" sub="3 profiles · default for scans" value="On"
            onClick={() => window.openInfo({ title: 'Family allergen session', sub: 'Pick which profiles every scan checks against by default. Maya, Theo and Ava are in this household.', lines: 3 })} />
          <SRow icon="card" label="Plan" sub="Free · 20 scans / month" value="Manage" last
            onClick={() => window.openInfo({ title: 'Plan', sub: 'You’re on Free — 20 scans a month. Plan tiers and pricing are still being decided.', lines: 3 })} />
        </SGroup>
        ...
        <SGroup title="Mode">
          <SRow icon="people" label="Child mode" sub={isChild ? `On for ${active} — simpler, warmer screens` : 'Simplified screens for child profiles'}
            ...
            onClick={() => window.openInfo({ title: 'Child mode', sub: 'When on for a profile, results use simpler, warmer language. It’s set per profile — child profiles get it by default.', lines: 3 })} />
        </SGroup>
        ...
        <SGroup title="Privacy & data">
          <SRow icon="shield" label="Privacy summary" sub="What we store, in plain words"
            onClick={() => window.openInfo({ title: 'Privacy summary', sub: 'A plain-words explanation of what’s stored and what never leaves your device. Final wording is owned by the content library.', lines: 4 })} />
          <SRow icon="download" label="Export my diary" sub="Download everything as a file"
            onClick={() => window.openInfo({ title: 'Export my diary', sub: 'Download your full scan history and profile as a file you keep.', lines: 3 })} />
          <SRow icon="flag" label="Report a labeling error"
            onClick={() => window.openInfo({ title: 'Report a labeling error', sub: 'Flag something we read wrong so we can fix the database.', lines: 3 })} />
          <SRow icon="clock" label="Database" sub="Current as of 28 May 2026" value="Updated" last
            onClick={() => window.openInfo({ title: 'Database', sub: 'When the ingredient database last refreshed, and controls for downloading updates.', lines: 3 })} />
        </SGroup>
