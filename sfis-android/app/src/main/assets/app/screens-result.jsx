...
function ResultScreen({ child = false, setChild = () => {}, onBack = () => {}, variant = 'tint',
  product = DEMO.product, brand = DEMO.brand, findings = DEMO.findings, unverified = DEMO.unverified, session = null }) {
  const [card, setCard] = React.useState(null); // { finding, item }
  const open = (finding, item) => setCard({ finding, item });
  const nothing = (!findings || !findings.length);
      ...
      <div className="fs-scroll" style={{ flex: 1, padding: '4px 18px 26px' }}>
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: child ? 18 : 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: T.serif, fontWeight: 600, fontSize: child ? 22 : 20, lineHeight: 1.1, color: T.ink, letterSpacing: -.3 }}>{product}</div>
            <div style={{ fontFamily: T.sans, fontSize: 13.5, color: T.ink2, marginTop: 3 }}>{brand} · scanned just now</div>
          </div>
        </div>

        {/* family session indicator (only when scanning for multiple profiles) */}
        {session && session.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: T.sans, fontSize: 12.5, fontWeight: 600, color: T.ink3 }}>This session</span>
            {session.map((p) => <ProfileTag key={p.name} name={p.name} isChild={p.child} />)}
          </div>
        )}
        ...
        <div style={{ fontFamily: T.serif, fontSize: child ? 21 : 18, lineHeight: 1.35, color: T.ink, margin: '2px 0 16px' }}>
          {nothing ? (child ? 'Nothing here from your list — but always check the pack.' : 'Nothing in this product matched your profile. Always check the packaging too.')
                   : (child ? 'Here’s what’s inside that you told us about.' : 'Here’s what we found that matches your profile.')}
        </div>

        {/* match bars */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {findings.map((f, i) => <MatchBar key={i} data={f} variant={variant} child={child} onOpen={open} />)}
        </div>

        {/* could not verify */}
        {!child && unverified && unverified.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <CouldNotVerify data={unverified} />
          </div>
        )}
