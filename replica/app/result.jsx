    ...
    <div>
      ...
      {data.map((g, gi) => (
        <div key={gi} style={{ marginBottom: gi < data.length - 1 ? 14 : 0 }}>
          ...
          {g.connectivity && (
            <button className="fs-press" onClick={() => window.openInfo({ title: 'Check for an update', sub: 'Look online for fresh database info on these ingredients.', lines: 2 })} style={{ marginTop: 10, display: 'inline-flex', alignItems: 'center', gap: 7,
...
function ResultFooter() {
  return (
    <div style={{ marginTop: 4 }}>
      ...
      <button className="fs-press" onClick={() => window.openInfo({ title: 'Report a labeling error', sub: 'Tell us if something on this label was read wrong.', lines: 3 })} style={{ marginTop: 12, fontFamily: T.sans, fontSize: 13.5, fontWeight: 600,
